...
### `send_midi_usb_note_on <NOTE> [VELOCITY] [CHANNEL]`
Sends a MIDI Note On message over USB.
- `NOTE`: MIDI note number `0–127`.
- `VELOCITY`: velocity `0–127` (default: `127`).
- `CHANNEL`: MIDI channel `0–15` (default: `0`).

Min Firmware Version: 250

### `send_midi_usb_note_off <NOTE> [VELOCITY] [CHANNEL]`
Sends a MIDI Note Off message over USB.
- `NOTE`: MIDI note number `0–127`.
- `VELOCITY`: velocity `0–127` (default: `0`).
- `CHANNEL`: MIDI channel `0–15` (default: `0`).
...
### `charge_restricted`
Prints whether the device is restricted from playing while charging (a factory-configured setting). Takes no arguments.
